...
temp_accuracy = 0.9973273942093541		
# ===== FIX FOR LOCAL RUN (DO NOT REMOVE) =====

# Ensure text column exists
if "clean_joined" not in df.columns:
    if "text" in df.columns:
        df["clean_joined"] = df["text"].astype(str)
    elif "title" in df.columns:
        df["clean_joined"] = df["title"].astype(str)
    else:
        df["clean_joined"] = df.iloc[:, 0].astype(str)

# Ensure label column exists
if "isfake" not in df.columns:
    if "label" in df.columns:
        df["isfake"] = df["label"]

# ===== END FIX =====
# 							#paste outputs of previous file
# FIX: create clean_joined column if missing
if "clean_joined" not in df.columns:
    if "text" in df.columns:
        df["clean_joined"] = df["text"].astype(str)
    elif "title" in df.columns:
...
# FIX: ensure label column exists
if "isfake" not in df.columns and "label" in df.columns:
    df["isfake"] = df["label"]
